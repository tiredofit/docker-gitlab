...
[![Build Status](https://img.shields.io/docker/build/tiredofit/gitlab.svg)](https://hub.docker.com/r/tiredofit/gitlab)
[![Docker Pulls](https://img.shields.io/docker/pulls/tiredofit/gitlab.svg)](https://hub.docker.com/r/tiredofit/gitlab)
[![Docker Stars](https://img.shields.io/docker/stars/tiredofit/gitlab.svg)](https://hub.docker.com/r/tiredofit/gitlab)
[![Docker Layers](https://images.microbadger.com/badges/image/tiredofit/gitlab.svg)](https://microbadger.com/images/tiredofit/gitlab)
[![Image Size](https://img.shields.io/microbadger/image-size/tiredofit/gitlab.svg)](https://microbadger.com/images/tiredofit/gitlab)

- [Introduction](#introduction)
    - [Changelog](CHANGLOG.md)
- [Issues](#issues)
- [Announcements](https://github.com/tiredofit/docker-gitlab/issues/39)
...
If using the latest docker version and/or disabling selinux does not fix the issue then please file a issue request on the [issues](https://github.com/tiredofit/docker-gitlab/issues) page.
...
To illustrate linking with a postgresql container, we will use the [tiredofit/postgresql](https://github.com/tiredofit/postgresql) image. When using postgresql image in production you should mount a volume for the postgresql data store. Please refer the [README](https://github.com/tiredofit/postgresql/blob/master/README.md) of postgresql for details.
...
sudo chcon -Rt svirt_sandbox_file_t /data/gitlab/postgresql
```
...
To illustrate linking with a redis container, we will use the [tiredofit/redis](https://github.com/tiredofit/redis) image. Please refer the [README](https://github.com/tiredofit/redis/blob/master/README.md) of redis for details.
...
Copy the `ca.crt` file into the certs directory on the [datastore](#data-store). The `ca.crt` file should contain the root certificates of all the servers you want to trust. With respect to GitLab CI, this will be the contents of the gitlab_ci.crt file as described in the [README](https://github.com/tiredofit/docker-gitlab-ci/blob/master/README.md#ssl) of the [docker-gitlab-ci](https://github.com/tiredofit/docker-gitlab-ci) container.
...
If you are using the [docker-redmine](https://github.com/tiredofit/docker-redmine) image, you can *one up* the gitlab integration with redmine by adding `--volumes-from=gitlab` flag to the docker run command while starting the redmine container.
...
| `MODE` | Runtime Mode `START` `INIT` `SANITIZE` `RAKE`. Defaults to `START` |
| `RAKE_TASK` | If `MODE` is set above, execute this command - See next section |
...
| `NGINX_HSTS_ENABLED` | Advanced configuration option for turning off the HSTS configuration. Applicable only when SSL is in use. Defaults to `true`. See [#138](https://github.com/tiredofit/docker-gitlab/issues/138) for use case scenario. |
...
docker-compose run --rm gitlab -e MODE=RAKE -e RAKE_TASK=gitlab:env:info
docker-compose run --rm gitlab -e MODE=RAKE -e RAKE_TASK=gitlab:import:repos
